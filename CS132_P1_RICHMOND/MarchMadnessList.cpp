...
void marchMadnessList::printTeams(int howMany) {
	...
	cout << left << setw(5) << "Rank" << setw(20) << "School" << setw(8) << "Points" << setw(8) << "Record" << setw(8) << "Previous\n\n";
...
/////////////////////////////////////////////////////////////////////////////////////////
//Function Name:	outpurData(ofstream& output)
...
void marchMadnessList::outputData(ofstream& output) {
	...
	for (list<TeamData>::iterator i = this->teamList.begin(); i != this->teamList.end(); ++i) {
		...
		output << i->position << '\t' << i->name << '\t' << i->wins << '-' << i->losses << '\t' << i->points << '\t' << i->lastPosition << '\t';
	}
